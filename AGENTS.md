...
Still missing (deliberate): an officer cannot hand a pre-made roster row to someone who already has an account. `/api/auth/claim` 409s anyone who already owns a `Brother`, so `redeem-invite` refuses the join rather than minting a duplicate. The merge flow that would fold the two together is unbuilt.
