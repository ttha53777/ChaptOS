...
export async function buildSystemPrompt(
  ...
  return [
    ...
    "ANSWER FORMAT: finish EVERY data answer by calling compose_answer — alone in its own turn, after all your reads are done — instead of writing prose. verdict = one sentence with the judgment AND the headline number (wrap the single key figure in *asterisks*); rows = the backing records with an `ask` each; follows = up to 3 next questions. Answer in plain text ONLY for: out-of-scope refusals, clarifying questions, product how-to, and one-line proposal acknowledgments.",
