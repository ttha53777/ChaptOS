...
// ─── WidgetHideButton ─────────────────────────────────────────────────────
// Admin-only "hide this widget" affordance, reused across the dashboard's
// toggleable widgets. Reveals on hover of an ancestor marked `group relative`
// (mirrors the edit pencil on AnnouncementCard). stopPropagation keeps a click
// from also triggering a clickable widget's drawer. The caller is responsible
// for rendering it only for admins.
export function WidgetHideButton({ onHide, label }: { onHide: () => void; label: string }) {
  return (
    <button
      onClick={(e) => { e.stopPropagation(); onHide(); }}
      aria-label={`Hide ${label}`}
      title={`Hide ${label}`}
      className="absolute right-2 top-2 z-10 flex h-6 w-6 items-center justify-center rounded-md border border-white/[0.08] bg-[#10121a]/80 text-slate-400 opacity-0 transition-opacity hover:border-red-500/40 hover:bg-red-500/10 hover:text-red-300 group-hover:opacity-100 focus-visible:opacity-100"
    >
