    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <button onClick={() => actions.setActiveDrawer("treasury")} className="flex flex-col gap-1 rounded-xl card-premium px-3 py-2.5 text-left active:border-white/[0.14]">
          <div className="flex items-center gap-1 text-[#958d7c]">
          ...
          <span className="text-[17px] font-bold tabular-nums text-[#a78bfa]">{fmt$(liveBalance)}</span>
          <span className="text-[10px] text-[#958d7c]">projected {fmt$(liveProjected)}</span>
        </button>
        <button onClick={() => actions.setActiveDrawer("door")} className="flex flex-col gap-1 rounded-xl card-premium px-3 py-2.5 text-left active:border-white/[0.14]">
          ...
          <span className="text-[17px] font-bold tabular-nums text-[#d98ba3]">{fmt$(totalDoorRev)}</span>
          <span className="text-[10px] text-[#958d7c]">{bestEvent ? `best ${fmt$(bestEvent.doorRevenue)}` : "—"}</span>
        </button>
      </div>
      ...
      >
        <div className="flex items-center justify-between border-b border-white/[0.07] px-4 py-3">
          <h2 className="text-[13px] font-semibold text-[#ece7dd]">Party Events</h2>
          <div className="flex items-center gap-2">
            <p className="text-[15px] font-bold text-[#ece7dd]">{fmt$(totalDoorRev)}</p>
            <button onClick={(e) => { e.stopPropagation(); actions.setActiveModal("revenue"); }} className="rounded-md bg-[#a78bfa]/15 px-2 py-0.5 text-[10px] font-semibold text-[#a78bfa] active:bg-[#a78bfa]/25 transition-colors">+ Add</button>
          </div>
        </div>
        <div className="space-y-3 px-4 py-4">
          {partyList.length === 0 ? (
            <p className="py-4 text-center text-[12px] text-[#958d7c]">No events logged — tap + Add to log revenue</p>
          ) : partyList.map(e => {
            ...
            return (
              <div key={e.id} className="flex items-center gap-3">
                <div className="w-20 shrink-0">
                  <p className={`truncate text-[12px] font-medium ${isTop ? "text-[#a78bfa]" : "text-[#c9c2b4]"}`}>{e.name}</p>
                </div>
                <div className="min-w-0 flex-1">
                  <div className="h-1.5 overflow-hidden rounded-full bg-white/[0.07]">
                    <div className={`h-full rounded-full transition-all duration-500 ${isTop ? "bg-[#a78bfa]" : "bg-white/[0.18]"}`} style={{ width: `${barPct}%` }} />
                  </div>
                </div>
                <span className="w-12 shrink-0 tabular-nums text-right text-[12px] font-semibold text-[#ece7dd]">{fmt$(e.doorRevenue)}</span>
              </div>
            );
          })}
        </div>
